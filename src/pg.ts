...
export default class PGStorageClient {
    private static readonly TABLES: Record<TableName, string> = {
        ...
        "channels": "CREATE TABLE channels (id BIGINT PRIMARY KEY, name VARCHAR(32));",
        "messages": "CREATE TABLE messages (id BIGINT PRIMARY KEY, channel_id BIGINT REFERENCES channels(id), user_id BIGINT REFERENCES users(id), content TEXT);",
    ...
    async writeChannel(id: Snowflake, name: string): Promise<void> {
        await this.client.query("INSERT INTO channels VALUES ($1, $2) ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name;", [id, name]);
    }

    async writeMessage(id: Snowflake, channelId: Snowflake, userId: Snowflake, content: string): Promise<void> {
        await this.client.query("INSERT INTO messages VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO UPDATE SET channel_id = EXCLUDED.channel_id, user_id = EXCLUDED.user_id, content = EXCLUDED.content;", [id, channelId, userId, content]);
    }
